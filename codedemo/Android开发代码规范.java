/**
Java代码装在到classloader中是一起装载的，因此，程序员在编写field, method, local时不需要考虑放置的位置，具有极大的自由性。但这种方式导致的结果大大降低了代码的阅读性。举个例子，假设一个Activity有10个method，onCreate和onResume放置在类的最后，那么每次想了解该activity的执行流程时，我们都会scroll the page to get the target or use ctrl+o to search for the method. 但这些方式都需要花时间，大大降低了效率。

因此为了能够方便后续人员更好地理解代码流程或让自己更快速理清代码结构，良好的编程规范势在必行。因此本文档从代码位置学角度制定相应的规范来避免阅读性差的代码。

规范：

field放在类的最前面
1. static final修饰的field放在一起
2. public 放在一起
3. private 放在一起
...
前面的方法体现本类的功能
中期的方法按照调用顺序放置
后期的方法提供为外界调用（如静态方法，set and get method）
 */
